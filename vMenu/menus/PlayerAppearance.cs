...
namespace vMenuClient.menus
{
    public class PlayerAppearance
    {
        ...
        private void CreateMenu()
        {
            ...
            var savedPedsBtn = new MenuItem("查看已保存人物", "编辑、重命名、克隆、生成或删除已保存的人物模型.") { Label = "→→→" };
            ...
            var addonPedsBtn = new MenuItem("附加人物模型", "从附加人物模型列表中生成一个人物模型.") { Label = "→→→" };
            ...
            var walkingStyle = new MenuListItem("行走风格", walkstyles, 0, "更改当前人物的行走风格. 每次更改玩家模型或加载已保存的人物时,都需要重新应用此设置.");
            ...
            var selectedSavedPedMenu = new Menu("已保存人物", "renameme");
            ...
            var replaceSavedPed = new MenuItem("~r~替换已保存的人物", "用当前的人物替换这个已保存的人物.注意,此操作不可撤销!") { LeftIcon = MenuItem.Icon.WARNING };
        ...
        private void RefreshCustomizationMenu()
        {
            ...
            for (var drawable = 0; drawable < 12; drawable++)
            {
                ...
                if (maxVariations > 0)
                {
                    ...
                    var drawableTextures = new MenuListItem($"{textureNames[drawable]}", drawableTexturesList, currentDrawable, $"使用 ← & → 选择 ~o~{textureNames[drawable]} 衣服~s~, 键下 ~r~ENTER~s~ 循环浏览可用的纹理.");
            ...
            for (var tmpProp = 0; tmpProp < 5; tmpProp++)
            {
                ...
                if (maxPropVariations > 0)
                {
                    ...
                    var propTextures = new MenuListItem($"{propNames[tmpProp]}", propTexturesList, currentProp + 1, $"使用 ← & → 选择 ~o~{propNames[tmpProp]} 配饰~s~, 键下 ~r~ENTER~s~ 循环浏览可用的纹理.");
