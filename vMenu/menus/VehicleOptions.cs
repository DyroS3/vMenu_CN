...
namespace vMenuClient.menus
{
    public class VehicleOptions
    {
        ...
        private void CreateMenu()
        {
            ...
            var radioStations = new MenuListItem("载具默认电台", stationNames, radioIndex, "选择在生成载具时设置的默认电台");
            ...
            var setDirtLevel = new MenuListItem("设定脏污等级", dirtlevel, 0, "选择载具上的污垢可见度, 键下 ~r~ENTER~s~ " + "确认更改.");
            ...
            #region Submenus
            // Submenu's
            VehicleModMenu = new Menu("载具改装", "载具改装");
            ...
            VehicleDoorsMenu = new Menu("载具车门", "载具车门管理");
            VehicleWindowsMenu = new Menu("载具车窗", "载具车窗管理");
            VehicleComponentsMenu = new Menu("载具改件", "载具附件/改件");
            VehicleLiveriesMenu = new Menu("载具涂装", "载具涂装");
            VehicleColorsMenu = new Menu("载具配色", "载具配色");
            ...
            DeleteConfirmMenu.OnItemSelect += (sender, item, index) =>
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            Notify.Alert("如果要删除载具, 您必须坐在驾驶座上.");
                        }
            ...
            // primary menu
            var primaryColorsMenu = new Menu("载具配色", "主要颜色");
            ...
            // secondary menu
            var secondaryColorsMenu = new Menu("载具配色", "次要颜色");
            ...
            var fwu = new MenuItem("~y~↑~s~ 升起前窗", "升起前面的车窗.");
            var fwd = new MenuItem("~o~↓~s~ 降下前窗", "降下前面的车窗.");
            var rwu = new MenuItem("~y~↑~s~ 升起后窗", "升起后面的车窗.");
            var rwd = new MenuItem("~o~↓~s~ 降下后窗", "降下后面的车窗.");
                    ...
                    if (veh != null && veh.Exists() && !veh.IsDead && veh.Driver == Game.PlayerPed)
                    {
                        ...
                        for (var extra = 0; extra < 14; extra++)
                        {
                            ...
                            if (veh.ExtraExists(extra))
                            {
                                ...
                                var extraCheckbox = new MenuCheckboxItem($"改件 #{extra}", extra.ToString(), veh.IsExtraOn(extra));
        ...
        public void UpdateMods(int selectedIndex = 0)
        {
            ...
            if (veh != null && veh.Exists() && !veh.IsDead)
            {
                ...
                var vehicleWheelType = new MenuListItem("轮毂类型", wheelTypes, MathUtil.Clamp(GetVehicleWheelType(veh.Handle), 0, 11), $"选择适合您载具的~y~轮毂类型.");
                ...
                var lowGripTires = new MenuCheckboxItem("降低轮胎抓地力", "启用或禁用 ~y~ 降低轮胎抓地力 ~s~s.", GetDriftTyresEnabled(veh.Handle));
